...
So, here, Collectd is configured as a server by listening from anywhere at the common port for Collectd clustering. Moreover, the "write_graphite" plugin is loaded in order to work as an output to the Graphite's Carbon tool, which is the storage backend of a Graphite installation.