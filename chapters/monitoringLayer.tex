...
The pluguin is called "collectd-docker" and its documentation can be found in \href{https://github.com/bobrik/collectd-docker/}{GitHub} (--REFERENCE--). But, in order to follow the idea of clustering Collectd it has been modified in order to use the network plugin instead the write graphite plugin. So, the fact of being running on the same O.S. it requires changing the UDP ports through the metrics are sent by avoiding port binding issues. Moreover, previous Collectd client is also configured as a proxy server by re-configuring the network plugin as shown next: 
...
\end{verbatim}
\item Collectd docker socket API reader (from the O.S. point of view):
\begin{verbatim}
